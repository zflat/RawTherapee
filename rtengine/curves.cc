...
WavretigainCurve::WavretigainCurve() : sum(0.f) {};

void WavretigainCurve::Reset()
{
    lutWavretigainCurve.reset();
    sum = 0.f;
}

void WavretigainCurve::Set(const Curve &pCurve)
{
    if (pCurve.isIdentity()) {
        Reset(); // raise this value if the quality suffers from this number of samples
        return;
    ...
    lutWavretigainCurve(501); // raise this value if the quality suffers from this number of samples
    sum = 0.f;

    for (int i = 0; i < 501; i++) {
        lutWavretigainCurve[i] = pCurve.getVal(double(i) / 500.);

        if(lutWavretigainCurve[i] < 0.02f) {
            lutWavretigainCurve[i] = 0.02f;    //avoid 0.f for wavelet : under 0.01f quasi no action for each value
        }

        sum += lutWavretigainCurve[i];
    }

    //lutWavCurve.dump("wav");
}
void WavretigainCurve::Set(const std::vector<double> &curvePoints)
{
...
WavstyCurve::WavstyCurve() : sum(0.f) {};

void WavstyCurve::Reset()
{
    lutWavstyCurve.reset();
    sum = 0.f;
}
...
void WavstyCurve::Set(const Curve &pCurve)
{
    ...
    lutWavstyCurve(501); // raise this value if the quality suffers from this number of samples
    sum = 0.f;

    for (int i = 0; i < 501; i++) {
        lutWavstyCurve[i] = pCurve.getVal(double(i) / 500.);

        if(lutWavstyCurve[i] < 0.02f) {
            lutWavstyCurve[i] = 0.02f;    //avoid 0.f for wavelet : under 0.01f quasi no action for each value
        }

        sum += lutWavstyCurve[i];
    }
...
void WavmergCurve::Reset()
{
    lutWavmergCurve.reset();
    sum = 0.f;
}

void WavmergCurve::Set(const Curve &pCurve)
{
    if (pCurve.isIdentity()) {
        Reset(); // raise this value if the quality suffers from this number of samples
        return;
    ...
    lutWavmergCurve(501); // raise this value if the quality suffers from this number of samples
    sum = 0.f;

    for (int i = 0; i < 501; i++) {
        lutWavmergCurve[i] = pCurve.getVal(double(i) / 500.);

        if(lutWavmergCurve[i] < 0.02f) {
            lutWavmergCurve[i] = 0.02f;    //avoid 0.f for wavelet : under 0.01f quasi no action for each value
        }

        sum += lutWavmergCurve[i];
    }

    //lutWavCurve.dump("wav");
}
void WavmergCurve::Set(const std::vector<double> &curvePoints)
{
...
Wavmerg2Curve::Wavmerg2Curve() : sum(0.f) {};

void Wavmerg2Curve::Reset()
{
    lutWavmerg2Curve.reset();
    sum = 0.f;
}

void Wavmerg2Curve::Set(const Curve &pCurve)
{
    if (pCurve.isIdentity()) {
        Reset(); // raise this value if the quality suffers from this number of samples
        return;
    ...
    lutWavmerg2Curve(501); // raise this value if the quality suffers from this number of samples
    sum = 0.f;

    for (int i = 0; i < 501; i++) {
        lutWavmerg2Curve[i] = pCurve.getVal(double(i) / 500.);

        if(lutWavmerg2Curve[i] < 0.02f) {
            lutWavmerg2Curve[i] = 0.02f;    //avoid 0.f for wavelet : under 0.01f quasi no action for each value
        }

        sum += lutWavmerg2Curve[i];
    }

    //lutWavCurve.dump("wav");
}
void Wavmerg2Curve::Set(const std::vector<double> &curvePoints)
{
