...
public:
    ...
    void Lab_Local(int call, int sp, float** shbuffer, LabImage* original, LabImage* transformed, int sx, int sy, int cx, int cy, int oW, int oH,  int fw, int fh, bool locutili, int sk, const LocretigainCurve & locRETgainCcurve, bool locallutili, LUTf & lllocalcurve, double &hueref, double &chromaref, double &lumaref);
...
//    void strcuv_data (std::string retistr, int *s_datc, int &siz);
    ...
    void ColorLight_Local(int call, LabImage * bufcolorig, LabImage * bufcoltra, int sp, float moy, const float hueplus, const float huemoins, const float hueref, const float dhue, const float chromaref, const float lumaref, LUTf & lllocalcurve, const local_params& lp, float **deltE, LabImage* original, LabImage* transformed, int cx, int cy);
