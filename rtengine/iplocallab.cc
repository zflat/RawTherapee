...
void ImProcFunctions::Contrast_Local(float pm, struct local_contra &lco, float lumaref, float av, const struct local_params& lp, LabImage* original, LabImage* transformed, int cx, int cy)
{
    BENCHFUN
...
void ImProcFunctions::InverseColorLight_Local(const struct local_params& lp, LabImage* original, LabImage* transformed, int cx, int cy, const LUTf & localcurve)
{
    ...
    for (int y = 0; y < transformed->H; y++) {
        ...
        for (int x = 0; x < transformed->W; x++) {
            ...
            switch(zone) {
            case 0: { // outside selection and outside transition zone => no effect, keep original values
                transformed->L[y][x] = localcurve[original->L[y][x]];  //apply lightness
